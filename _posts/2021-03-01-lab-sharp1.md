...
## 文本内容查看工具
  ...
  1 cat [Option] FILE_NAME or FILE_PATH
  2 tac [OPtion] FILE_NAME or FILE_PATH (从最后一行依次显示内容)
  3 more [Option] FILE_NAME or FILE_PATH `-n` 加上数字
  4 less [Option] FILE_NAME or FILE_PATH与 more 类似 但大多数时用到 less, Example: cat install.list | less 就像查看 manual 一样，`/` 查找关键字
  5 head [Option] FILE_NAME or FILE_PATH `-n` X (显示 X 行)
